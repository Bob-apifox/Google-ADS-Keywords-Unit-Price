...
def execute_fix():
    ...
    if ad_pause_ops:
        print(f"Pausing {len(ad_pause_ops)} identical active ads...")
    ...
    print(">>> Uploading 3 Distinct RSAs with Tracking Suffix...")
    rsa_ops = []
    ...
    for c_name, (ag_id, ag_name) in ag_map.items():
        ...
        headlines = cfg["rsa"]["headlines"]
        descriptions = cfg["rsa"]["descriptions"]
        
        # We create 3 distinct sets of assets
        # Ad 1: indices 0-7 headlines, 0-1 descriptions
        # Ad 2: indices 4-11 headlines, 1-2 descriptions
        # Ad 3: indices 8-14 + 0 headlines, 2-3 descriptions
        
        sets = [
            (headlines[0:8], descriptions[0:2]),
            (headlines[4:12], descriptions[1:3]),
            (headlines[8:15] + [headlines[0]], descriptions[2:4] if len(descriptions) >= 4 else [descriptions[0], descriptions[-1]])
        ]
        
        for i, (hls, descs) in enumerate(sets):
    ...
    if rsa_ops:
        print(f">>> Uploading {len(rsa_ops)} Distinct RSAs...")
        req = client.get_type('MutateAdGroupAdsRequest')
        req.customer_id = CUSTOMER_ID
    ...
    print("[SUCCESS] Fix Execution Finished!")
    return True
