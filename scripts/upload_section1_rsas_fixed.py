...
# Headlines max 30 bytes, Descriptions max 90 bytes (CJK characters count as 2)
def get_len(text):
...
SECTION_1_CONFIG = {
    ...
    "Google-Sa-CP-TR": {
        ...
        "final_url": "https://apidog.com/compare/apidog-vs-postman/",
        "rsa": {
            "headlines": ["En İyi Postman Alternatifi", "Hepsi Bir Arada API Platformu", "Ücretsiz API Testi ve Mocking", "Tek Tıkla Postman Aktarımı", "Otomatik API Dokümantasyonu", "Ücretsiz Takım Çalışması", "Sınırsız Test Çalıştırma", "API Geliştirme Aracı", "Apidog Türkçe Destekli", "API Tasarım ve Test Platformu", "Ücretsiz Postman Alternatifi", "OpenAPI 3.1 Desteği", "Akıllı Mock Server Araçları", "Postman Koleksiyon Aktarımı", "Ücretsiz API İstemcisi"],
            "descriptions": ["Postman yerine Apidog kullanın. API tasarımı, testi, mock ve dokümantasyonu tek araçta.", "Postman koleksiyonlarınızı tek tıkla aktarın. Hemen ücretsiz başlayın.", "Sınırsız test çalıştırma ve ücretsiz takım çalışması ile verimliliğinizi artırın.", "Türkiye'deki yazılımcılar için modern ve güçlü API geliştirme ve test platformu."],
            "path1": "compare", "path2": "postman"
        }
    }
}
