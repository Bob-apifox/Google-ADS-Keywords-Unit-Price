...
def main():
    print("==================================================================")
    print("🚀 DYNAMIC EXECUTION: Section 1 (12 Single-Country Campaigns Building)")
    print("==================================================================")
    ...
    # STEP 0: Dynamically Query Account Campaigns to get exact IDs for all 12
    print("\n>>> STEP 0: Dynamically Mapping Campaign Names to IDs...")
    ...
    for batch in stream:
        for row in batch.results:
            account_camps[row.campaign.name] = row.campaign.id

    camp_id_map = {}
    for name in SECTION_1_CONFIG.keys():
        if name in account_camps:
            camp_id_map[name] = account_camps[name]
            print(f"Mapped Campaign: '{name}' -> ID: {account_camps[name]}")
        else:
            print(f"⚠️ Campaign '{name}' NOT FOUND in account!")

    if len(camp_id_map) == 0:
        print("❌ ERROR: No matching campaigns found!")
        return

    # STEP 1: Enable Campaigns & Set Tracking Suffix
    print(f"\n>>> STEP 1: Enabling {len(camp_id_map)} Campaigns & Attaching Tracking Suffix...")
    camp_ops = []
    for camp_name, cid in camp_id_map.items():
        camp_op = client.get_type('CampaignOperation')
        campaign = camp_op.update
        campaign.resource_name = ga_service.campaign_path(CUSTOMER_ID, cid)
    ...
    # STEP 2: Pause Old Ad Groups
    print(f"\n>>> STEP 2: Pausing Old Enabled Ad Groups in {len(camp_id_map)} Campaigns...")
    camp_ids_str = ", ".join([str(cid) for cid in camp_id_map.values()])
    ag_query = f"""
    ...
    new_ag_ops = []
    for camp_name, cid in camp_id_map.items():
        ag_name = SECTION_1_CONFIG[camp_name]["new_ag_name"]
        ag_op = client.get_type('AdGroupOperation')
        ag = ag_op.create
        ag.name = ag_name
        ag.campaign = ga_service.campaign_path(CUSTOMER_ID, cid)
    ...
    try:
        ...
        for result in response.results:
            ...
            st_query = f"SELECT ad_group.id, ad_group.name FROM ad_group WHERE ad_group.id = {ag_id}"
            st = ga_service.search_stream(customer_id=CUSTOMER_ID, query=st_query)
            for b in st:
                for r in b.results:
