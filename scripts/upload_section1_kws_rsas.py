...
SECTION_1_CONFIG = {
    ...
    "Google-Sa-CP-PT": {
        ...
        "rsa": {
            "headlines": ["Alternativa ao Postman - Apidog", "Plataforma de API Tudo-em-Um", "Testar API Online Grátis", "Importação Postman em 1 Clique", "Documentação e Mock de API", "Design de API Colaborativo", "Sem Limites de Execução", "Cliente HTTP e Testes API", "Apidog em Português Grátis", "Gerador de Docs API Grátis", "Postman Grátis Alternativa", "Workspace de API em Equipe", "Testes de API Automatizados", "Importar Coleções do Postman", "Apidog Brasil e Portugal"],
            "descriptions": ["Substitua o Postman pelo Apidog. Teste, documente e simule APIs em uma só plataforma. Grátis.", "Migre suas coleções do Postman sem perdas. Sem limites de execução em equipe.", "Testes de API, servidores mock e documentação em um só lugar. Comece grátis hoje.", "Ferramenta API nativa em português para desenvolvedores no Brasil e Portugal."],
            "path1": "compare", "path2": "postman"
        }
    },
    "Google-Sa-CP-JP": {
        ...
        "rsa": {
            "headlines": ["Postmanの代替ツール - Apidog", "オールインワンAPI開発プラットフォーム", "無料APIテスト＆モックサーバー", "Postmanデータ1クリック移行", "APIドキュメント自動生成", "チームコラボレーション無料", "実行回数無制限のAPIテスト", "日本語対応API開発ツール", "Apidog 日本語公式", "API設計・テスト・文書化", "Postman 無料 代替", "OpenAPI 3.1 完全対応", "APIモックサーバー自動作成", "Postmanコレクション完全移行", "無料APIクライアントツール"],
    ...
    "Google-Sa-CP-KR": {
        ...
        "rsa": {
            "headlines": ["Postman 최고의 대체 툴 - Apidog", "올인원 API 개발 플랫폼", "무료 API 테스트 및 목 서버", "Postman 1클릭 마이그레이션", "자동 API 문서 생성기", "팀 협업 완전 무료", "실행 제한 없는 API 테스트", "한국어 지원 API 개발 툴", "Apidog 공식 한국어", "API 설계 및 테스트 통합", "Postman 무료 대체 솔루션", "OpenAPI 3.1 지원", "스마트 API 목 서버", "Postman 컬렉션 손실없이 이전", "무료 API 클라이언트"],
    ...
    "Google-Sa-CP-TW": {
        ...
        "rsa": {
            "headlines": ["Postman 最佳替代方案 - Apidog", "一站式 API 開發與測試平台", "免費 API 測試與 Mock 服務", "1 鍵匯入 Postman 資料", "自動生成 API 文件", "團隊協作完全免費", "無執行次數限制的 API 測試", "繁體中文 API 開發工具", "Apidog 繁體中文版", "API 設計與調試整合", "Postman 免費替代軟體", "OpenAPI 3.1 原生支援", "智慧 API Mock 伺服器", "Postman 集合無縫轉移", "免費 API 用戶端工具"],
    ...
    "Google-Sa-CP-VN": {
        ...
        "rsa": {
            "headlines": ["Công Cụ Thay Thế Postman tốt nhất", "Nền Tảng API Tất-Cả-Trong-Một", "Kiểm Thử API Miễn Phí", "Chuyển Dữ Liệu Postman 1 Click", "Tự Động Tạo Tài Liệu API", "Cộng Tác Nhóm Miễn Phí", "Không Giới Hạn Lượt Chạy Test", "Công Cụ API Tiếng Việt", "Apidog Tiếng Việt Official", "Thiết Kế & Kiểm Thử API", "Thay Thế Postman Miễn Phí", "Hỗ Trợ OpenAPI 3.1 Native", "Tạo Smart Mock Server API", "Nhập Collection Postman Dễ Dàng", "Công Cụ API Client Miễn Phí"],
            "descriptions": ["Thay thế Postman bằng Apidog. Thiết kế, kiểm thử, mock và tạo document API trên 1 nền tảng.", "Nhập bộ sưu tập Postman dễ dàng chỉ với 1 click. Sử dụng miễn phí ngay.", "Cộng tác nhóm miễn phí không giới hạn số lượt chạy test script. Nâng cao năng suất.", "Nền tảng phát triển API hỗ trợ tiếng Việt dành riêng cho lập trình viên Việt Nam."],
            "path1": "compare", "path2": "postman"
        }
    },
    "Google-Sa-CP-ID": {
    ...
    "Google-Sa-CP-DE": {
        ...
        "rsa": {
            "headlines": ["Postman Alternative 2026 - Apidog", "All-in-One API Platform", "Kostenloses API Testing", "1-Klick Postman Migration", "OpenAPI 3.1 Native Platform", "Ohne Runner Ausführungslimits", "Automatische API Dokus", "Datenschutz & SOC2 Konform", "Postman Alternative Tool", "Kostenlose Team Kollaboration", "Automatisierte API Tests", "Smart Mock API Server", "Postman Kollektionen Import", "Integrierter API Workspace", "Apidog API Platform Kostenlos"],
            "descriptions": ["Ersetzen Sie Postman durch Apidog. API-Design, Testing, Mocking und Doku in einer Plattform.", "Importieren Sie Postman-Kollektionen nahtlos mit 1 Klick. Jetzt kostenlos testen.", "Kostenlose Team-Kollaboration ohne Beschränkungen bei Testausführungen.", "Moderne API-Entwicklungsplattform für Entwickler in Deutschland, Österreich & Schweiz."],
            "path1": "compare", "path2": "postman"
        }
    },
    "Google-Sa-CP-FR": {
        ...
        "rsa": {
            "headlines": ["Alternative à Postman - Apidog", "Plateforme API All-in-One", "Test API & Mock Gratuit", "Migration Postman en 1-Clic", "Documentation API Automatique", "Collaboration d'Équipe Gratuite", "Sans Limites d'Exécution", "Client HTTP & Test API", "Alternative Postman Gratuite", "Générateur de Docs API", "Workspace API Tout-en-Un", "Support Native OpenAPI 3.1", "Mock Server API Intelligente", "Import de Collections Postman", "Plateforme API Gratuite"],
            "descriptions": ["Remplacez Postman par Apidog. Concevez, testez et documentez vos API sur une seule plateforme.", "Importez vos collections Postman en 1 clic sans perte de données. Essayez gratuitement.", "Tests d'API automatisés et serveurs mock sans limites d'exécution pour votre équipe.", "Outil de développement API moderne pour les développeurs en France et Europe."],
            "path1": "compare", "path2": "postman"
        }
    },
    "Google-Sa-CP-TR": {
        ...
        "rsa": {
            "headlines": ["En İyi Postman Alternatifi - Apidog", "Hepsi Bir Arada API Platformu", "Ücretsiz API Testi ve Mocking", "Tek Tıkla Postman Aktarımı", "Otomatik API Dokümantasyonu", "Ücretsiz Takım Çalışması", "Çalıştırma Sınırı Olmayan Test", "API Geliştirme Aracı", "Apidog Türkçe Destekli", "API Tasarım ve Test Platformu", "Ücretsiz Postman Alternatifi", "OpenAPI 3.1 Desteği", "Akıllı Mock Server Araçları", "Postman Koleksiyon Aktarımı", "Ücretsiz API İstemcisi"],
            "descriptions": ["Postman yerine Apidog kullanın. API tasarımı, testi, mock ve dokümantasyonu tek araçta.", "Postman koleksiyonlarınızı tek tıkla aktarın. Hemen ücretsiz başlayın.", "Sınırsız test çalıştırma ve ücretsiz takım çalışması ile geliştirici verimliliğini artırın.", "Türkiye'deki yazılımcılar için modern ve güçlü API geliştirme ve test platformu."],
            "path1": "compare", "path2": "postman"
        }
    }
}
...
def execute_operations():
    ...
    print(">>> Fetching the 10 Ad Groups...")
    query = "SELECT ad_group.id, ad_group.name FROM ad_group WHERE ad_group.name LIKE '%Postman-Alternative%-2026%'"
    ...
    for camp_name, cfg in SECTION_1_CONFIG.items():
        ...
        if ag_name not in ag_map:
            print(f"Skipping {ag_name}, not found.")
            continue
    ...
    kw_req.operations.extend(kw_ops)
    kw_req.partial_failure = True
    kw_resp = agc_service.mutate_ad_group_criteria(request=kw_req)
    
    if kw_resp.partial_failure_error and kw_resp.partial_failure_error.details:
        for error in kw_resp.partial_failure_error.details:
            print(f"Keyword Partial Failure: {error}")
    else:
        print(f"Keywords Success! Uploaded {len(kw_resp.results)} keywords.")
    
    # Upload RSAs
    print(f">>> Uploading {len(rsa_ops)} RSAs...")
    ...
    if rsa_resp.partial_failure_error and rsa_resp.partial_failure_error.details:
        ...
        print(f"RSAs Success! Uploaded {len(rsa_resp.results)} RSAs.")
